                  ...
                  <tbody className="divide-y divide-white/5">
                    {loadingUsers ? (
                      ...
                      filteredUsers.map(user => {
                        const isActive = user.last_sign_in_at && (new Date() - new Date(user.last_sign_in_at)) / (1000 * 60 * 60 * 24) <= 30;
                        return (
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-2">
                                ...
                                {user.provider === 'google' && <span className="bg-white/10 text-white text-[10px] font-bold px-1.5 py-0.5 rounded border border-white/20 flex items-center gap-1"><svg className="w-3 h-3" viewBox="0 0 24 24"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>Google</span>}
                                {(!user.provider || user.provider === 'email') && <span className="bg-white/10 text-white text-[10px] font-bold px-1.5 py-0.5 rounded border border-white/20">Email</span>}
                              ...
                              <div className="flex items-center gap-3 text-xs text-gray-400">
                                ...
                                <span className="flex items-center gap-1" title="Active within last 30 days">
                                  <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-gray-500'}`}></span>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {loadingActivity ? (
                      ...
                      activityLogs.map((log) => {
                        const isError = log.action.toLowerCase().includes('fail') || log.action.toLowerCase().includes('error') || log.action.toLowerCase().includes('ban');
                        const isSuccess = log.action.toLowerCase().includes('complete') || log.action.toLowerCase().includes('success');
                        
                        let displayDetails = log.details || '';
                        let extractedUrl = null;
                        if (displayDetails.includes('URL: http')) {
                          const match = displayDetails.match(/URL:\s*(https?:\/\/[^\s|]+)/);
                        ...
                        <tr key={log.id} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm">
                            <span className={`px-2.5 py-1 rounded-lg ${isError ? 'bg-red-500/20 text-red-400 border border-red-500/30' : isSuccess ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-white/10 text-gray-300'}`}>
                              {log.action}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-gray-400 text-sm break-all max-w-md">
                            <div className="flex flex-col gap-2">
                              <div className="max-h-24 overflow-y-auto custom-scrollbar">{displayDetails}</div>
                              {extractedUrl && (
                                <button onClick={() => { navigator.clipboard.writeText(extractedUrl); toast.success("URL Copied!"); }} className="flex items-center gap-1 w-fit px-2 py-1 bg-white/5 hover:bg-white/10 rounded border border-white/10 text-xs text-purple-300 transition-colors">
