...
const VideoDownloader = () => {
  ...
  const handlePodcastAction = async (filename, thumbnail) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/visualizer', { filename, thumbnail });
      const newFilename = res.data.filename;
      const link = document.createElement('a');
      link.href = `http://localhost:5000/api/serve/${newFilename}`;
  ...
  const handleSubtitleAction = async (url) => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/subtitles', { url });
      const newFilename = res.data.filename;
      const link = document.createElement('a');
      link.href = `http://localhost:5000/api/serve/${newFilename}`;
      link.download = newFilename;
  ...
  const handleConvertAction = async (filename, format) => {
    setConverting(filename);
    try {
      const res = await axios.post('http://localhost:5000/api/convert', { filename, format });
      const newFilename = res.data.filename;
      const link = document.createElement('a');
      link.href = `http://localhost:5000/api/serve/${newFilename}`;
      link.download = newFilename;
  ...
  const handleNativeShare = async (filename, title) => {
    const shareUrl = `http://${networkIp}:5000/api/serve/${filename}`;
    if (navigator.share) {
      try {
  ...
  const handleDeleteHistory = async (link) => {
    try {
      // Extract filename from the link (e.g., http://localhost:5000/api/serve/filename.mp4)
  ...
  const startDownload = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/download', payload);
  ...
  const pollAllTasks = async (taskIds) => {
    ...
    while (active.length > 0) {
      for (let tid of active) {
        try {
          const res = await axios.get(`http://localhost:5000/api/progress/${tid}`);
          const data = res.data;
          ...
          if (data.status === 'completed') {
            ...
            const link = `http://localhost:5000/api/serve/${data.filename}`;
          ...
          >
            ...
            >
              ...
              <div className="p-4 bg-white rounded-2xl shadow-inner">
                <QRCodeSVG value={`http://${networkIp}:5000/api/serve/${showQR}`} size={200} />
              </div>
            </motion.div>
          </motion.div>
        )}
                    ...
                    {videoInfo.has_subtitles && (
                       <button 
                         onClick={async () => {
                           try {
                             toast.loading('Extracting Subtitles...', { id: 'subs' });
                             const res = await axios.post('http://localhost:5000/api/subtitles', { url: videoInfo.url || url });
                             const link = document.createElement('a');
                             link.href = `http://localhost:5000/api/serve/${res.data.filename}`;
                    ...
                    {data.status === 'completed' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-6 flex flex-col gap-4">
                        ...
                        <div className="w-full bg-black/60 rounded-xl overflow-hidden border border-white/10 mb-2 shadow-inner">
                          <video 
                            src={`http://localhost:5000/api/serve/${data.filename}`} 
                            autoPlay 
                            muted 
                            loop 
                        ...
                        <div className="flex gap-2 flex-wrap justify-end">
                          ...
                          <a href={`http://localhost:5000/api/serve/${data.filename}`} download className="px-6 py-2 bg-green-500 hover:bg-green-600 transition-colors rounded-lg text-sm font-bold text-white shadow-lg flex items-center gap-2 ml-auto">
