...
const VideoDownloader = () => {
  ...
  useEffect(() => {
    axios.get('https://media-backend-zyw5.onrender.com/api/network')
  ...
  const handlePodcastAction = async (filename, thumbnail) => {
    ...
    try {
      const res = await axios.post('https://media-backend-zyw5.onrender.com/api/visualizer', { filename, thumbnail });
  ...
  const handleSubtitleAction = async (url) => {
    try {
      ...
      const res = await axios.post('https://media-backend-zyw5.onrender.com/api/subtitles', { url });
      const newFilename = res.data.filename;
      const link = document.createElement('a');
      link.href = `https://media-backend-zyw5.onrender.com/api/serve/${newFilename}`;
      link.download = newFilename;
  ...
  const handleReadTranscript = async (url) => {
    try {
      ...
      const res = await axios.post('https://media-backend-zyw5.onrender.com/api/transcript', { url });
  ...
  const handleNativeShare = async (filename, title) => {
    const shareUrl = `https://media-backend-zyw5.onrender.com/api/serve/${filename}`;
    if (navigator.share) {
      try {
  ...
  const handleDeleteHistory = async (link) => {
    try {
      // Extract filename from the link (e.g., https://media-backend-zyw5.onrender.com/api/serve/filename.mp4)
      const filename = link.split('/').pop();
      await axios.delete(`https://media-backend-zyw5.onrender.com/api/delete/${filename}`);
  ...
  const handleFetchInfo = async (e) => {
    ...
    try {
      const res = await axios.post('https://media-backend-zyw5.onrender.com/api/info', { url });
  ...
  const startBatchDownload = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post('https://media-backend-zyw5.onrender.com/api/download', {
        urls, quality
      });
  ...
  const startDownload = async () => {
    ...
    try {
      ...
      const res = await axios.post('https://media-backend-zyw5.onrender.com/api/download', payload);
  ...
  const pollAllTasks = async (taskIds) => {
    ...
    while (active.length > 0) {
      for (let tid of active) {
        try {
          const res = await axios.get(`https://media-backend-zyw5.onrender.com/api/progress/${tid}`);
          const data = res.data;
          ...
          if (data.status === 'completed') {
            ...
            const link = `https://media-backend-zyw5.onrender.com/api/serve/${data.filename}`;
          ...
          >
            ...
            >
              ...
              <div className="p-4 bg-white rounded-2xl shadow-inner">
                <QRCodeSVG value={`https://media-backend-zyw5.onrender.com/api/serve/${showQR}`} size={200} />
              </div>
            </motion.div>
          </motion.div>
        )}
                    ...
                    {videoInfo.has_subtitles && (
                       <button 
                         onClick={async () => {
                           try {
                             toast.loading('Extracting Subtitles...', { id: 'subs' });
                             const res = await axios.post('https://media-backend-zyw5.onrender.com/api/subtitles', { url: videoInfo.url || url });
                             const link = document.createElement('a');
                             link.href = `https://media-backend-zyw5.onrender.com/api/serve/${res.data.filename}`;
                    ...
                    {data.status === 'completed' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-6 flex flex-col gap-4">
                        ...
                        <div className="w-full bg-black/60 rounded-xl overflow-hidden border border-white/10 mb-2 shadow-inner">
                          <video 
                            src={`https://media-backend-zyw5.onrender.com/api/serve/${data.filename}`} 
                            autoPlay 
                            muted 
                            loop 
                        ...
                        <div className="flex gap-2 flex-wrap justify-end">
                          ...
                          <a href={`https://media-backend-zyw5.onrender.com/api/serve/${data.filename}`} download className="px-6 py-2 bg-green-500 hover:bg-green-600 transition-colors rounded-lg text-sm font-bold text-white shadow-lg flex items-center gap-2 ml-auto">
